...
func AddrAdd2(sock *nl.NetlinkSocket, link Link, addr *Addr) error {

	req := nl.NewNetlinkRequest(syscall.RTM_NEWADDR, syscall.NLM_F_CREATE|syscall.NLM_F_EXCL|syscall.NLM_F_ACK)
	req.Sock = sock
	return addrHandle2(link, addr, req)
}
...
func AddrDel2(sock *nl.NetlinkSocket, link Link, addr *Addr) error {
	req := nl.NewNetlinkRequest(syscall.RTM_DELADDR, syscall.NLM_F_ACK)
	req.Sock = sock
	return addrHandle2(link, addr, req)
}
...
func addrHandle2(link Link, addr *Addr, req *nl.NetlinkRequest) error {
	base := link.Attrs()
	if addr.Label != "" && !strings.HasPrefix(addr.Label, base.Name) {
		return fmt.Errorf("label must begin with interface name")
	}
	ensureIndex2(req.Sock, base)
...
func AddrList2(sock *nl.NetlinkSocket, link Link, family int) ([]Addr, error) {
	req := nl.NewNetlinkRequest(syscall.RTM_GETADDR, syscall.NLM_F_DUMP)
	req.Sock = sock
	...
	msgs, err := req.Execute(syscall.NETLINK_ROUTE, syscall.RTM_NEWADDR)
	if err != nil {
		return nil, fmt.Errorf("Error in AddrList2: %v", err)
	}

	index := 0
	if link != nil {
		base := link.Attrs()
		ensureIndex2(sock, base)
		index = base.Index
	}
